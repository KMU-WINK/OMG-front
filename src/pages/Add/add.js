...
function Add(props) {
  ...
  const HandleGlassCount = (e) => {
    ...
    if (
      (soju === 0 && target === "desSoju") ||
      (beer === 0 && target === "desBeer") ||
      (etc === 0 && target === "desEtc") ||
      (soju === 30 && target === "incSoju") ||
      (beer === 30 && target === "incBeer") ||
      (etc === 30 && target === "incEtc")
    ) {
  ...
  const [check3, setCheck3] = useState([
    { color: "#ffffff", background: "#009800" },
    "✅",
  ]); //비밀번호 존재 확인
  ...
  const setChoice1 = (e) => {
    //공병상태 확인1
    if (check1[1] === "❎")
      setCheck1([{ color: "#ffffff", background: "#009800" }, "✅"]);
  ...
  const checkAdd = () => {
    ...
    if (fullAddress === "여기를 눌러 수거받을 주소를 입력해주세요") {
      ...
      setAddError(
        "공동현관 비밀번호를 입력하거나 비밀번호 없이 출입이 가능해야 돼요"
      );
      openAddErrorModal();
    } else {
      openAddModal();
  ...
  return (
    <style.Wrap>
      ...
      <style.Wrap2>
        <style.title>
          <h1>어떤 종류의 공병을 가지고 있나요?</h1>
        </style.title>
        <style.banners>
          <BottleBanner
            style1
            onClick={openSojuModal}
            btnName={["소주병", <br />, soju, "병"]}
          />
          <BottleBanner
            style2
            onClick={openBeerModal}
            btnName={["맥주병", <br />, beer, "병"]}
          />
          <BottleBanner
            style3
            onClick={openEtcModal}
            btnName={["기타", <br />, etc, "병"]}
          />
        </style.banners>

        <style.title>
          <h1>공병의 상태를 확인해주세요</h1>
        </style.title>
        <style.choiceButtons>
          <WhiteFullButton
            style={check1[0]}
            onClick={setChoice1}
            btnName={[check1[1], " 모든 공병 내부에 이물질이 없습니다"]}
          />
          <WhiteFullButton
            style={check2[0]}
            onClick={setChoice2}
            btnName={[check2[1], " 모든 공병에 깨진 부분이 없습니다"]}
          />
        </style.choiceButtons>
        ...
        <style.title>
          <h1>공동현관 비밀번호를 입력해주세요</h1>
        </style.title>
        <style.choiceButtons>
          <WhiteFullButton
            style={check3[0]}
            onClick={setChoice3}
            btnName={[check3[1], " 공동현관 비밀번호가 있습니다"]}
          />
          <WhiteFullButton
            style={check3[2]}
            btnName={[<b>공동현관 비밀번호 | </b>, password]}
      ...
      <Modal2
        open={sojuModalOpen}
        close={closeSojuModal}
        header="소주병 개수"
        button1={closeSojuModal}
        button1Content="확인"
      >
        <style.count>
          <style.countButton onClick={HandleGlassCount} id={"desSoju"}>
            -
          </style.countButton>
          소주병
          <br />
          {soju}병
          <style.countButton onClick={HandleGlassCount} id={"incSoju"}>
            +
          </style.countButton>
        </style.count>
      </Modal2>
      ...
      <Modal2
        open={beerModalOpen}
        close={closeBeerModal}
        header="맥주병 개수"
        button1={closeBeerModal}
        button1Content="확인"
      >
        <style.count>
          <style.countButton onClick={HandleGlassCount} id={"desBeer"}>
            -
          </style.countButton>
          맥주병
          <br />
          {beer}병
          <style.countButton onClick={HandleGlassCount} id={"incBeer"}>
            +
          </style.countButton>
        </style.count>
      </Modal2>
      ...
      <Modal2
        ...
        close={closeEtcModal}
        header="기타병 개수"
        button1={closeEtcModal}
        button1Content="확인"
      >
        <style.count>
          <style.countButton onClick={HandleGlassCount} id={"desEtc"}>
            -
          </style.countButton>
          기타
          <br />
          {etc}병
          <style.countButton onClick={HandleGlassCount} id={"incEtc"}>
            +
          </style.countButton>
        </style.count>
      </Modal2>
      ...
      <Modal4
        open={bottleErrorModalOpen}
        close={closeBottleErrorModal}
        header="확인해 주세요"
        button1={closeBottleErrorModal}
        button1Content="확인"
      >
      ...
      <Modal1
        ...
        header="공동현관 비밀번호 입력"
        button1={closePasswordModal}
        button2={onClickPasswordModal}
        button1Content="취소"
        button2Content="확인"
      >
        <style.Input>
      ...
      <Modal4
        ...
        close={closeAddModal}
        header="공병등록 완료!"
        button1={closeAddModal}
        button1Content="홈으로 돌아가기"
      >
        공병 등록을 완료했어요!
        <br />
        확인 수정 삭제는 등록내역을 확인해주세요!
      </Modal4>
    </style.Wrap>
  );
}
